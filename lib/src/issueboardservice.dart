...
class IssueBoardService {
    ...
    Future<IssueBoard> getAllIssues(PageInfo pageInfo) async {
        ...
        return new IssueBoard.create(pageInfo, #getAllIssues, [], issues.skip(pageInfo.skipCount).take(pageInfo.pageSize).toList());
    }
    ...
    Future<IssueBoard> getIssuesByName(String name, PageInfo pageInfo) async {
        ...
        return new IssueBoard.create(pageInfo, #getIssuesByName, [name], issueFilter.skip(pageInfo.skipCount).take(pageInfo.pageSize).toList());
    }
    ...
    Future<IssueBoard> getIssuesByProjectName(String projectName, PageInfo pageInfo) async {
        ...
        var project = (await store.getAllProjects()).singleWhere((Project project) => project.name.toLowerCase() == projectName.toLowerCase());

        var issues = (await store.getAllIssues()).where((Issue issue) => issue.projectName == project.name).toList();

        return new IssueBoard.create(pageInfo, #getIssuesByProjectName, [projectName], issues.skip(pageInfo.skipCount).take(pageInfo.pageSize).toList());
    }
}
