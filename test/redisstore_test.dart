...
void main() {
    ...
    group('Redis store integration tests', () {
        ...
        setUp(() {
            ...
            return RedisClient.connect(connectionString).then((RedisClient c) {
                client = c;
                client.select(1);
            });
        ...
        tearDown(() {
            ...
            return client.flushdb().then((c) => client.close()).then((c) => client = null);

        });

        test('serialise and store Issue object with correct key', () async {
            // Arrange
            var issue = new Issue.create('123', 'Lorem ipsum dolor sit amet', 'This is the issue description.', new DateTime.now(), new IssueStatus.opened(), 'Project 1', 'Bob Doe', 'Bob Doe');
        ...
        test('retrieves all Issue objects stored in the db ordered by issue id', () async {
            ...
            for (var i = 0; i < numberOfIssues; i++) {
                var issue = new Issue.create('$i', 'Lorem ipsum dolor sit amet', 'This is the issue description.', new DateTime.now(), new IssueStatus.opened(), 'Project 1', 'Bob Doe', 'Bob Doe');
